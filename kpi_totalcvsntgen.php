      ...
      <div class="row ">
        <div class="justify-content-center ">
          ...
          <div class="card">
            <div class="card-body">
                <h1> </h1><br>
              <h2 style="text-align:center;"  ><b>KPI - Total CV Sent</b></h2>
               <div >
               <form action="kpi_totalcvsntexprt.php" method="post">
    ...
    $qry_run = mysqli_query($conn, $qry);
      if (mysqli_num_rows($qry_run) > 0) {
        while ($row = mysqli_fetch_array($qry_run)) {
            ?>
                <tr><th colspan="6" style="border-bottom: 2px solid black;text-align:center;font-size: 23px;"><b><?php echo $row['name'] ?>'S Total CV Sent <?php echo $frmDate; ?> - <?php echo $toDate; ?> </b></th></tr>
           <?php
        }
...
if (isset($_GET['generate_report'])) {
    ...
    $qry_run = mysqli_query($conn, $qry);
      if (mysqli_num_rows($qry_run) > 0) {
        while ($row = mysqli_fetch_array($qry_run)) {
            ?>
                <tr><th colspan="6" style="border-bottom: 2px solid black;text-align:center;font-size: 23px;"><b> Total Count of CV'S Sent : <?php echo $row['total_count']; ?> </b></th></tr>
           <?php
        }
...
<?php
 $qry = "SELECT COUNT(*) AS total_count 
    ...
    INNER JOIN clintmanege ON (`candidate_summery`.`clint_id` = `clintmanege`.`clint_id`) 
    INNER JOIN users ON (`candidate_summery`.`user_id` = `users`.`id`) 
    WHERE candidate_summery.`user_id` = $empID AND candidate_summery.feedback = 'ClientReview' 
    AND candidate_summery.`date` BETWEEN '$frmDate' AND '$toDate'
    GROUP BY candidate_summery.application_id 
    ORDER BY candidate_summery.id ASC) AS subquery";
    ...
    if (mysqli_num_rows($qry_run) > 0) {
        while ($row = mysqli_fetch_array($qry_run)) {
            ...
            ?>
 <tr >
                <td colspan="5" style="border-top: 2px solid black;border-bottom: 2px solid black;text-align:center;border-left: 2px solid black;"><b>Total Count OF Sent CV's</b></td>
