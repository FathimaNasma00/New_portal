...
<form action="" id="manage-upload" autocomplete="off" enctype="multipart/form-data" method="post">
      ...
      <div class="row mb-3">
					...
					<div class="form-group">
							...
							<select name="recruiter" id="recruiter" required class="custom-select select2  custom-select-sm form-control form-control-sm">
                            <option></option>
                            <?php if (isset($_SESSION['login_type']) && in_array($_SESSION['login_type'], [1, 2, 3, 5])) {  ?>
                                
                                <?php 
              	$employees = $conn->query("SELECT `recruiter` FROM `users` WHERE type IN (1, 2, 3) AND id NOT IN (5, 19, 25,39,50) AND active =1");
              	...
              	<?php endwhile; ?>
	                           <?php }elseif($_SESSION['login_type'] == 4){ ?> 
	                                <?php 
              	$employees = $conn->query("SELECT `recruiter` FROM `users` WHERE type = 4 AND active =1");
              	while($row= $employees->fetch_assoc()):
              	?>
              	<option value="<?php echo $row['recruiter'] ?>" <?php echo isset($recruiter) && in_array($row['recruiter'],explode(',',$recruiter)) ? "selected" : '' ?>><?php echo ucwords($row['recruiter']) ?></option>
              	<?php endwhile; ?>
