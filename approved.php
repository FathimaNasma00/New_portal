			...
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					...
					while($row = $user->fetch_assoc()){
						...
						$where = " where status=1 AND user_id = '{$_SESSION['login_id']}' ";
					endif;
					$qry = $conn->query("SELECT * FROM documents where status=1 AND user_id = '{$_SESSION['login_id']}' order by unix_timestamp(date_created) desc ");
